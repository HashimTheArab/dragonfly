...
func NewEndCrystal(opts world.EntitySpawnOpts) *world.EntityHandle {
	showBase := true
	if opts.EndCrystalShowBase != nil {
		showBase = *opts.EndCrystalShowBase
	}
	return opts.New(EndCrystalType, endCrystalBehaviour{showBase: showBase})
}
...
func (endCrystalType) DecodeNBT(m map[string]any, data *world.EntityData) {
	b := endCrystalBehaviour{showBase: true}
	if _, ok := m["ShowBottom"]; ok {
		b.showBase = nbtconv.Bool(m, "ShowBottom")
	}
