...
func (c Candle) UseOnBlock(pos cube.Pos, face cube.Face, _ mgl64.Vec3, tx *world.Tx, user item.User, ctx *item.UseContext) bool {
	if existing, ok := tx.Block(pos).(Candle); ok {
		...
		// Candle clusters keep the clicked block's lit state. The item placed is
		// folded into the same block state, so only the dye/colour must match.
...
func (c Candle) BreakInfo() BreakInfo {
	return newBreakInfo(0.1, alwaysHarvestable, nothingEffective, simpleDrops(item.NewStack(Candle{Dyed: c.Dyed, Colour: c.Colour}, c.AdditionalCount+1)))
}
