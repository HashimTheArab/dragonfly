...
func (c CoralFan) UseOnBlock(pos cube.Pos, face cube.Face, _ mgl64.Vec3, tx *world.Tx, user item.User, ctx *item.UseContext) bool {
	...
	attachFace := cube.FaceDown
	if c.Hanging {
		attachFace = c.Facing.Opposite().Face()
	}
	attach := pos.Side(attachFace)
	if !tx.Block(attach).Model().FaceSolid(attach, attachFace.Opposite(), tx) {
		return false
	}
...
func allCoralFan() (c []world.Block) {
	f := func(dead bool) {
		for _, t := range CoralTypes() {
			c = append(c, CoralFan{Type: t, Dead: dead, Facing: cube.North})
			c = append(c, CoralFan{Type: t, Dead: dead, Facing: cube.East})
