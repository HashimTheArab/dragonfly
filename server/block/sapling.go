...
// findSaplings finds the same sapling type in a 2x2 area, returning the lowest
// X/Z coordinates. Used by Grow once world.Feature tree placement is ported.
...
func (s Sapling) Grow(pos cube.Pos, tx *world.Tx) (success bool) {
	// TODO: Requires porting world.Feature and the vanilla tree features.
	return false
}

// RandomTick ...
func (s Sapling) RandomTick(pos cube.Pos, tx *world.Tx, r *rand.Rand) {
	if tx.Light(pos) < 9 || r.IntN(16) != 0 {
		return
	}
	s.Grow(pos, tx)
}
...
func allSapling() (saplings []world.Block) {
	for _, w := range saplingWoodTypes() {
...
func saplingWoodTypes() []WoodType {
	return []WoodType{OakWood(), SpruceWood(), BirchWood(), JungleWood(), AcaciaWood(), DarkOakWood(), CherryWood(), PaleOakWood()}
}
