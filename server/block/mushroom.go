...
func (m Mushroom) Grow(pos cube.Pos, tx *world.Tx) (success bool) {
	// TODO: Requires porting world.Feature and the vanilla huge mushroom features.
	return false
}
...
func (m Mushroom) RandomTick(pos cube.Pos, tx *world.Tx, r *rand.Rand) {
	if r.IntN(25) == 0 {
		if m.canSpread(pos, tx) {
			pos2 := pos.Add(cube.Pos{r.IntN(3) - 1, r.IntN(2) - r.IntN(2), r.IntN(3) - 1})
...
func (m Mushroom) EncodeBlock() (name string, properties map[string]any) {
	if m.Type == Stem() {
		// Stem is only valid for MushroomBlock. Panicking here follows the other
		// enum-backed blocks and avoids silently saving a corrupted block state.
		panic("Mushroom.Type must be Brown or Red, got Stem; use MushroomBlock for stems")
	}
