...
func (s SnowLayer) BreakInfo() BreakInfo {
	return newBreakInfo(0.2, alwaysHarvestable, shovelEffective, func(_ item.Tool, enchantments []item.Enchantment) []item.Stack {
		if hasSilkTouch(enchantments) {
			return []item.Stack{item.NewStack(s, 1)}
		}
		return []item.Stack{item.NewStack(item.Snowball{}, s.Height+1)}
	})
}
